...
void UpdateSession::onMediaClosed() {
  ...
  // Waiting for close ends here, with either a commit or a question.
  evaluate();
  ...
  if (state_.phase == UpdateState::Phase::Confirming && elapsed_ >= delay_) return;
...
void UpdateSession::evaluate() {
  ...
  switch (decision.action) {
    case Action::Allow:
      ...
      } else if (trigger_ == UpdateTrigger::OnPlayerClose && !closed_) {
        state.phase = Phase::WaitingForClose;
      } else if (askToConfirm_) {
